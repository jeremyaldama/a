...
BEGIN
	SELECT e.id_ejecutivo, e.DNI, e.nombre, e.apellido_paterno 
    FROM ejecutivo e 
    WHERE e.activo = 1 AND 
		(e.DNI LIKE CONCAT('%',_nombre_DNI,'%') OR 
		CONCAT(e.nombre,' ',e.apellido_paterno) LIKE CONCAT('%',_nombre_DNI,'%'));
END$
CREATE PROCEDURE LISTAR_PROGRAMAS_ACADEMICOS_X_NOMBRE_CLAVE(
	IN _nombre_clave VARCHAR(100)
)
BEGIN
	SELECT pa.id_programa_academico, pa.fid_tipo_programa_academico, pa.clave, 
		pa.nombre, c.cantidad_creditos, c.fecha_inicio, t.cantidad_horas, 
        t.fecha_realizacion 
    FROM programa_academico pa 
    LEFT JOIN curso c ON c.id_curso = pa.id_programa_academico 
    LEFT JOIN taller t ON pa.id_programa_academico = t.id_taller 
    WHERE (pa.nombre LIKE CONCAT('%',_nombre_clave,'%') OR 
		pa.clave LIKE CONCAT('%',_nombre_clave,'%')) 
        and pa.activo = 1;
END$
...
BEGIN
	INSERT INTO sede(fid_tipo_sede,fid_ejecutivo_responsable,
    nombre,direccion,fecha_inauguracion,cantidad_aulas,aforo_total,
    foto,tiene_auditorio,tiene_biblioteca,tiene_estacionamiento,
    tiene_salas_estudio,tiene_cafeteria,activo) 
    VALUES(_fid_tipo_sede,_fid_ejecutivo_responsable,_nombre,
    _direccion,_fecha_inauguracion,_cantidad_aulas,_aforo_total,
    _foto,_tiene_auditorio,_tiene_biblioteca,
    _tiene_estacionamiento,_tiene_salas_estudio,_tiene_cafeteria,
    1);
...
BEGIN
	SELECT s.id_sede, tp.id_tipo_sede, tp.nombre 
		as nombre_tipo_sede, s.nombre as nombre_sede, 
        s.direccion, s.fecha_inauguracion, s.cantidad_aulas, 
        s.aforo_total, s.foto, s.tiene_auditorio, 
        s.tiene_biblioteca, s.tiene_estacionamiento, 
        s.tiene_salas_estudio, s.tiene_cafeteria, e.id_ejecutivo, 
        e.DNI, e.nombre as nombre_ejecutivo, e.apellido_paterno 
        FROM sede s INNER JOIN tipo_sede tp ON 
		s.fid_tipo_sede = tp.id_tipo_sede INNER JOIN ejecutivo e 
        ON e.id_ejecutivo = s.fid_ejecutivo_responsable 
        WHERE s.activo = 1 AND s.nombre 
        LIKE CONCAT('%',_nombre,'%');
END$
